...
class Simulator(object):
    ...
    def generate_new_pool_data(self):
        ...
        curr_borrow_rates = torch.stack([pool["borrow_rate"] for _, pool in latest_pool_data.items()])
        curr_borrow_amounts = torch.stack([pool["borrow_amount"] for _, pool in latest_pool_data.items()])
        curr_reserve_sizes = torch.stack([pool["reserve_size"] for _, pool in latest_pool_data.items()])
        ...
        amounts = torch.clip(new_borrow_amounts, min=torch.zeros_like(curr_reserve_sizes), max=curr_reserve_sizes)  # Ensure borrow amounts do not exceed reserves
